%% Hongyi Su(UCID 30053908), Supervisor: Assoc. Prof. Dr. Hersh Gilbert, department of geoscience, University of Calgary
% Map limits
clear all; close all;
...
switch n
    case 1
      ...
      for i=1:length(amp_values)
        ...
        hold on
        hour = floor(time_start);%UTC time(-8 would be corresponding to BC local time)
        min_t = (time_start - floor(time_start))*60;
        tt = datestr(datenum(2019,07,day_start,hour,min_t,0),'mmm.dd,yyyy HH:MM:SS');
        wants = datestr(tt,30)
        title(strcat(wants,'__1-15HZ','___Energy level(median)','=',string(median(Tf.one))));
        xlabel('longitude');
        ylabel('latitude');
        colorbar;
